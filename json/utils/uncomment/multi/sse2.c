...
multi_sse_done:
  ...
  else
  {
    ...
    if (json_over (p, 0)) goto multi_sse_end;
    ...
    while (true)
    {
      ...
      if (likely (m != 0))
      {
        ...
        if (json_over (j, m)) goto multi_sse_end;

        goto multi_sse_done;
      }

      if (json_over (j, 16u)) goto multi_sse_end;
